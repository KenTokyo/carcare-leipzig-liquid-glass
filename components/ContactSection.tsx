            ...
            <div className="space-y-3">
              {tabs.map((tab) => {
                ...
                return (
                  ...
                  >
                    <div className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${isActive ? 'bg-white/15 text-white' : 'border border-gray-100 bg-gray-50 text-blue-600'}`}>
                      {iconMap[tab.iconName]}
                    </div>
                    <div className="min-w-0">
                      <div className="text-sm font-bold leading-tight">{tab.label}</div>
                      <div className={`mt-1 text-xs ${isActive ? 'text-white/75' : 'text-gray-600'}`}>{tab.description}</div>
                    </div>
                  </motion.button>
                );
              })}
