...
| 3.36 | **🔴 FEHLER, kein Zulieferungspunkt: Reparaturseiten öffnen das falsche Formular.** Wer auf einer Reparaturseite „… anfragen" klickt, landet im **Aufbereitungs**formular und bekommt Felder für Pflegepakete und Wunschtermin statt für Schadenart, Versicherung und Bilder. **Gemessen, nicht sechs oder sieben, sondern genau diese fünf:** `/dellenentfernung-leipzig`, `/felgenreparatur-leipzig`, `/autoglas-leipzig`, `/smart-repair-leipzig`, `/autolackierung-leipzig`. `/hagelschadenreparatur-leipzig` zeigt bereits korrekt auf `#contact-schaden`. `/fuhrparkservice-leipzig` ist ein sechster, aber anders gelagerter Fall: Das ist B2B und gehört ins Geschäftskundenformular. **Nicht die Beschriftungen sind falsch, sondern das Ziel** — „Dellenentfernung anfragen" ist richtig, nur führt es an die falsche Stelle. Lösungsvorschlag und Aufwand: siehe `docs/uebergabe-schleife-1.md`. | offen, Entscheidung |
